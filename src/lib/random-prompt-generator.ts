...
export type RandomType = "random" | "portrait" | "landscape";
...
export const GeneratePrompt = () => {
  const item = sessionStorage.getItem("random-state");
  const state: RandomParams =
    item != null
      ? JSON.parse(item)
      : {
          type: "random",
  ...
  const isPortraitPrompt = state.type === "portrait";

  // Get the selected portrait shot from the HTML select element
  const selectedPortraitShot = "Random Shot";

  // Randomly select a portrait shot type
  const randomizedPortraitShot =
    portraitShotOptions[Math.floor(Math.random() * portraitShotOptions.length)];

  // Check if the current generator is for landscapes
  const isLandscapesPrompt = state.type === "landscape";

  // Get the selected landscapes shot from the HTML select element
  const selectedLandscapesShot = "Random Shot";
  ...
  const isObjectsActive =
    state.options.includes("objects") && !isLandscapesPrompt;
  const isPlacesActive = isLandscapesPrompt || state.options.includes("places");
  const isArtistsActive = state.options.includes("artists");
  const isStylesActive = state.options.includes("styles");
  const isColorsActive = state.options.includes("colors");
  const isAdjectivesActive = state.options.includes("adjectives");
  const isElementsActive =
    state.options.includes("elements") && !isLandscapesPrompt;
  const isImproversActive = state.options.includes("improvers");
  const isPrefixesActive =
    state.options.includes("prefixes") && !isLandscapesPrompt;
  const isSuffixesActive =
    state.options.includes("suffixes") && !isLandscapesPrompt;
  ...
  for (let i = 0; i < numArtists; i++) {
    if (i < artists.length) {
      artistsPrompt += `${randomElement(
        artists.filter((artist) => !artistsPrompt.includes(artist))
      )}, `;
    }
  }
  ...
  for (let i = 0; i < numStyles; i++) {
    if (i < styles.length) {
      stylesPrompt += `${randomElement(
        styles.filter((style) => !stylesPrompt.includes(style))
      )}, `;
    }
  }
  ...
  if (!isLandscapesPrompt) {
    if (isPrefixePrompt && isPrefixesActive) {
      // if is a prefix prompt, add a random prefix
      prompt += `${randomElement(prefixes)} `;
    }

    if (isObjectsActive && Math.random() < 0.1) {
    ...
    if (isSuffixPrompt && isSuffixesActive) {
      // if is a suffix prompt, add a random suffix
      prompt += ` ${randomElement(suffixes)}`;
    }
  }
  ...
  if (isAdjectivesActive) {
    const adjective1 = randomElement(adjectives);
    prompt += `, ${adjective1}`;
    if (adjectives.length > 1) {
      prompt += `, ${randomElement(
        adjectives.filter((adjective) => adjective !== adjective1)
      )}`;
    }
  }
